...
import (
	...
	"github.com/luisbarufi/my-money-api/src/model/users/repository/entity/converter"
	"go.uber.org/zap"
)
...
func (ur *userRepository) FindUserByEmailRepository(email string) (
	...
	logger.Info(
		"Init FindUserByEmailRepository",
	...
	logger.Info(
		"FindUserByEmailRepository executed successfully",
...
func (ur *userRepository) FindUserByIDRepository(id uint64) (
	...
	logger.Info(
		"FindUserByIDRepository executed successfully",
		zap.String("id", fmt.Sprintf("%d", user.ID)),
		zap.String("journey", "findUserByID"),
	)
...
func (ur *userRepository) FindUserByEmailAndPasswordRepository(
	...
	logger.Info(
		"Init FindUserByEmailAndPasswordRepository",
	...
	if err != nil {
		logger.Error(
			"Error executing findUserByEmailAndPassword query",
			err,
			zap.String("journey", "findUserByEmailAndPassword"),
		)
	...
	logger.Info(
		"FindUserByEmailAndPasswordRepository executed successfully",
		zap.String("id", fmt.Sprintf("%d", user.ID)),
		zap.String("journey", "findUserByEmailAndPassword"),
	)
