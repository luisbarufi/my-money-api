...
func (uc *userControllerInterface) FindUserByEmail(c *gin.Context) {
	...
	if _, err := mail.ParseAddress(userEmail); err != nil {
		logger.Error("Error trying to validade email", err, zap.String("journey", "findUserByEmail"))
	...
	if userDomain.GetID() == 0 {
		logger.Info("User not found", zap.String("journey", "findUserByID"))
		errorMessage := rest_err.NewNotFoundError("User not found")
		c.JSON(errorMessage.Code, errorMessage)
